...
export default function DynamicBackground({mutedBg, ...props}) {
  ...
  const draw = () => {
    ...
    for(let i = 0; i < circles.length; i++){
      ...
      if (circle.y - circle.radius > canvas.height) circle.y = -circle.radius * 2;
      if (circle.x - circle.radius > canvas.width) circle.x = -circle.radius * 2;
      if (circle.speedX < 0 && circle.x + circle.radius * 2 < 0) circle.x = canvas.width + circle.radius;
      if (circle.speedY < 0 && circle.y + circle.radius * 2 < 0) circle.y = canvas.height + circle.radius;
  ...
  <>
    ...
    <canvas className={`${styles.canvas} ${mutedBg ? styles.canvasMuted : ''}`} ref={canvasRef} style={{ width, height }} {...rest} />
  </>
  )
}
