...
:::tip
Revisa la referencia del API para [Crear un consent action para REDEC](../../api/resources/create-redec-consent-action.api.mdx) para mas detalles.
:::
...
Primero, debes <a href="/docs/api/resources/create-action-token/" target="_blank">crear un `action_token`</a> con los campos de audio requeridos:

- **`redec_audio_reference`**: URL del archivo **MP3** que contiene la grabación del consentimiento
- **`redec_audio_hash`**: Hash SHA256 del archivo de audio en formato hexadecimal (minúsculas)

:::warning[Requisitos obligatorios]
1. **Ambos campos son obligatorios**: Si incluyes uno de estos campos, **debes incluir ambos**. No puedes enviar solo `redec_audio_reference` sin `redec_audio_hash` o viceversa.
...
<Tabs>
  <TabItem value="javascript" label="JavaScript" default>
    ...
    const response = await fetch('https://api.soyio.id/api/v1/redec/consent_actions', {
      ...
      body: JSON.stringify({
        action_token: actionToken,              // Token creado con redec_audio_reference y redec_audio_hash
        entity_nin: '12.345.678-9',
        executive_nin: '98.765.432-1',
  ...
  <TabItem value="python" label="Python">
    ...
    response = requests.post(
        ...
        json={
            'action_token': action_token,  # Token creado con redec_audio_reference y redec_audio_hash
            'entity_nin': '12.345.678-9',
            'executive_nin': '98.765.432-1',
            'user_reference': 'call_center_user_123'
        }
...
- **[RDC31](./rdc31.mdx)** - Informa cada acceso exitoso bajo consentimiento y genera el reporte semanal de accesos
- **[Configura webhooks](./rdc30.mdx#suscribete-al-evento-de-creacion-de-reportes)** - Recibe notificaciones cuando se registren consentimientos y se generen reportes
