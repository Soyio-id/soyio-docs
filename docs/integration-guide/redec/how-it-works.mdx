...
### API de accesos RDC31

El endpoint `POST /api/v1/redec/accesses` registra cada acceso exitoso a la base de deuda realizado bajo un consentimiento REDEC válido.

**Qué valida**:
- `redec_id` asociado a tu empresa
- vigencia del consentimiento al momento del acceso
- revocación previa al acceso
- duplicados exactos de fecha y hora

### API de reportes

Los endpoints de reportes te permiten:
- Crear reportes RDC30, RDC31 y RDC40 con rangos de fechas específicos
...
- `consent_action.redec_receipt_generated` - Recibo PDF de consentimiento REDEC generado y listo para descarga
- `report.created` - Nuevo reporte generado (verifica `kind: "rdc30"`, `kind: "rdc31"` o `kind: "rdc40"` en el payload)
- `redec.rdc40_no_changes` - Evento informativo cuando no hubo eventos que generen entradas RDC40 en el período (ver [Genera reportes RDC40](./rdc40.mdx))
...
```json title="Payload del webhook"
{
...
**Campos importantes**:
- `redec_receipt_pdf`: URL para descargar el PDF del recibo
- `redec_pdf_hash`: Hash SHA256 para verificar integridad del PDF
- `redec_id`: Identificador único del proceso REDEC
- `user_reference`: Tu referencia interna del usuario (si la proporcionaste)

:::tip[Sincronización semanal con CMF]
Usa el webhook `report.created` y verifica el campo `kind` en el payload para identificar reportes RDC30 y RDC31 y sincronizarlos semanalmente con la CMF según lo requiere la normativa.
:::
...
- Canal de captura (`digital`, `phone_call`, `in_person`)
...
1. **Registro del consentimiento** - Cuando registras un consentimiento REDEC, el sistema inicia la generación del recibo
2. **Generación automática** - El recibo PDF se genera en segundo plano
3. **Notificación por webhook** - Cuando el PDF está listo, recibes el webhook `consent_action.redec_receipt_generated`
4. **Descarga del recibo** - Usa el endpoint `/redec/consent_actions/{consent_action_id}/download_receipt` para descargar el PDF
...
- **Header SHA256**: Hash del PDF en el header `X-SHA256-Hash`
- **Body**: Contenido binario del PDF

Para más información sobre cómo descargar y trabajar con recibos, consulta la [guía de recibos REDEC](./receipts.mdx).
...
- **Canal** (`channel`): Método de captura (`digital`, `phone_call`, `in_person`)
...
El reporte RDC31 consolida los accesos exitosos a la base de deuda bajo consentimiento:

**Información incluida**:
- `redec_id` del consentimiento usado
- fecha y hora de cada acceso
- RUT del mandatario cuando aplica
- metadata de cuadratura para la carátula
...
Únicamente las acciones registradas con plantillas REDEC-compliant se incluirán en los reportes RDC30. Además, solo los accesos asociados a consentimientos válidos y vigentes podrán formar parte del RDC31.
:::
