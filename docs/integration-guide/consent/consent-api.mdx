...
| **Metadata de la acción** | ✅ Capturada automáticamente | ⚠️ Debes proporcionarla |
...
- **Metadata obligatoria**: Debes incluir toda la información contextual necesaria que permita rastrear la acción hasta tu sistema
...
**Importante**: Para una trazabilidad adecuada, incluye información contextual en `metadata`. Si el canal es `digital`, también debes incluir `contextual_data`:
...
:::info[Campos del action token]
- `origin`: Identifica el contexto de negocio donde se captura el consentimiento (ej. `"onboarding-producto-a"`, `"checkout-flow"`)
- `channel`: Canal de captura. Valores posibles: `digital`, `phone_call`, `in_person`
- `metadata` (opcional): Objeto JSON libre (max 8 KB) con información adicional de trazabilidad
- `contextual_data`: Requerido para el canal `digital`. Incluye `ip`, `user_agent` y `url`
:::
...
Una vez que el usuario haya otorgado su(s) consentimiento(s) y tengas el `action_token`, crea el registro de consentimiento desde tu backend. Tienes dos opciones:

#### Opción A: Crea una acción de consentimiento individual (`ConsentAction`)

Usa el [endpoint de creación de acción de consentimiento](../../api/resources/create-consent-action.api.mdx) para registrar un solo consentimiento.

```json title="POST /api/v1/consent_actions"
{
    "action_token": "<action_token_from_step_3>",
    "user_reference": "user_12345", // Opcional
    "metadata": { // Opcional
...
:::tip
Para revisar el payload completo y sus validaciones, consulta [Crear un consent action](../../api/resources/create-consent-action.api.mdx).
:::

#### Opción B: Crea un commit de varios consentimientos (`ConsentCommit`)

Si el usuario otorgó (o revocó) varios consentimientos en la misma sesión, registra todos en una sola llamada con el [endpoint de creación de commit de consentimientos](../../api/resources/create-consent-commit.api.mdx). Esto genera varios `ConsentAction` asociados a una única versión del acuerdo del usuario (`Agreement`) y una sola evidencia.

```json title="POST /api/v1/consent_commits"
{
    "user_reference": "user_12345", // Opcional
    "entity_id": "<entity_id>",  // Opcional
    "metadata": {                   // Opcional, se fusiona con la metadata de cada action token
        "session_id": "sess_123456789",
        "form_id": "registration_form_001"
    },
    "consent_actions": [            // Requerido
...
Un `ConsentCommit` puede mezclar acciones de otorgamiento (`grant`) y revocación (`revoke`) en una sola llamada. El campo `consent_actions` es obligatorio.

La resolución de la entidad funciona igual en ambas opciones: si no hay una sesión de entidad activa, Soyio busca o crea la entidad usando el `entity_id` o `user_reference` que proporciones.

## Metadata

Consulta la [guía de metadata en consentimientos](./consent-metadata) para entender cómo usar el campo `metadata` para enriquecer la trazabilidad de tus registros, cuándo enviarlo, y cómo se fusiona a lo largo del flujo.

## Mejores prácticas para trazabilidad

Al usar la API directamente asumes la responsabilidad sobre la evidencia de la acción del usuario. Estas recomendaciones te ayudan a cubrir esa responsabilidad:

1. **Incluye la mayor cantidad de información contextual posible** en `metadata` y `contextual_data` de ambos endpoints. Consulta los ejemplos en la [guía de metadata](./consent-metadata).
2. **Mantén logs detallados** de las interacciones del usuario en tu sistema
3. **Implementa un sistema de auditoría** que permita rastrear cada acción de consentimiento hasta tu sistema
4. **Documenta tu proceso** de captura de consentimiento para auditorías futuras
5. **Usa identificadores únicos** en `metadata` para facilitar la búsqueda posterior de evidencia
