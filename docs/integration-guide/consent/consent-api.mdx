...
Una vez que el usuario haya otorgado su(s) consentimiento(s) y tengas el `action_token`, crea el registro de consentimiento mediante el [endpoint de creación de acción de consentimiento](../../api/resources/create-consent-action.api.mdx). Opcionalmente, agrega `user_reference` para asociar el consentimiento con un usuario de tu sistema y `metadata` para enriquecer la trazabilidad.
...
```mermaid
graph LR
    A["metadata en<br/>action_token<br/>(paso 3)"] --> C["metadata final<br/>en ConsentAction"]
    B["metadata en<br/>consent_action<br/>(paso 4)"] --> C
```

<Tabs>
    <TabItem value="api" label="Via API" default>
        1. **Al crear el action token** (paso 3): envía `metadata` en el body del request
        2. **Al crear el consent action** (paso 4): envía `metadata` en el body del request
        3. Soyio fusiona ambos objetos. Si hay keys duplicadas, **la metadata del consent action tiene precedencia**
    </TabItem>
    <TabItem value="sdk" label="Via SDK">
        1. **En las opciones del SDK** (`consentOptions`): pasa `metadata` en la configuración del componente
        2. **Al crear el consent action** (paso 4): envía `metadata` en el body del request desde tu backend
        3. Soyio fusiona ambos objetos. Si hay keys duplicadas, **la metadata del consent action tiene precedencia**
    </TabItem>
</Tabs>
...
Usa `metadata` para asociar información útil para la trazabilidad de tu negocio, como:
...
Para garantizar una trazabilidad adecuada cuando usas la API directamente, te recomendamos incluir la mayor cantidad de información contextual posible:
...
Incluye información contextual en `metadata` y `contextual_data`:
...
Incluye metadata adicional del método de captura:
...
1. **Incluye la mayor cantidad de información contextual posible** en ambos endpoints:
   - En `metadata` y `contextual_data` del action token: datos específicos de la acción del usuario
   - En `metadata` del consent action: información del método de captura y contexto de negocio
