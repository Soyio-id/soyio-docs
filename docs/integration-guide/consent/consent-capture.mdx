...
3. Se genera un **token de acción** (`action token`) cada vez que el usuario haga click un checkbox de consentimiento
...
5. En base a estas acciones de consentimiento, el sistema automáticamente crea o actualiza los **Agreements** para el usuario. El [Agreement](../main/agreement.mdx) es la fuente de verdad que determina las finalidades de tratamiento autorizadas en tu empresa en tiempo real.
...
Usa este método para casos donde necesitas registrar el consentimiento en el momento de interacción del usuario. Por ejemplo, cuando estas solicitando el consentimiento para una finalidad adicional para datos que ya tienes en tu sistema.
...
:::important
Esta modalidad es parte de nuestros planes Premium. Consulta por los planes disponibles a [hola@soyio.id](mailto:hola@soyio.id).
:::

Si no puedes utilizar el SDK de Soyio, utiliza el módulo de consentimiento a través de la API de Soyio. Esta forma es ideal cuando quieres implementar tu propia interfaz de consentimiento o integrar Soyio a sistemas que tienen sus propias interfaces. Revisa la [guía de integración vía API](./consent-api.mdx) para más información.
...
2. Hace un commit de varios consentimientos (`ConsentCommit`)
...
### Opcion 2: Crea un commit de varios consentimientos (`ConsentCommit`)
...
- [Revocación](./consent-revocation.mdx) - Habilita un portal para que tus usuarios revoquen su consentimiento