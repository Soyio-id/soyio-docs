...
Usa el campo `metadata` para asociar información contextual a la acción de consentimiento (identificadores de sesión, campaña, versión de formulario, etc.). Consulta la [guía de metadata en consentimientos](./consent-metadata.mdx) para más detalles sobre cómo funciona el flujo de metadata.
:::
