...
- `complies`: el usuario cumple con todos los requerimientos del template.
...
Si el estado es `complies`, pero `last_consent_action_id` es `null`, revisa los campos relacionados de versión. Eso indica que el Agreement actual cumple con la última versión del template, aunque la última acción explícita de consentimiento puede haber quedado registrada en otra versión del mismo template.
    ...
    "compliance_statuses": [
        {
            ...
            "last_related_consent_action_id": "<last_related_consent_action_id>",
