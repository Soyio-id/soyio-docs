...
:::info
Por ahora, este servicio está disponible [**solo para el SDK Web**](https://github.com/Soyio-id/soy-io-widget).
:::
...
const privacyCenterOptions = {
  // ...
  consentControl: 'checkbox' // Usa checkboxes en lugar de switches
};
```
...
const privacyCenterOptions = {
  // ...
  consentMode: 'batch',
  showConfirmation: true // Muestra un diálogo de confirmación antes de guardar
};
```
...
`consentRetentionPeriod`: string (e.g. `'30 days'`, `'1 week'`)
