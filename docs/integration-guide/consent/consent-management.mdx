...
Si también habilitarás el flujo de ejercicio de derechos en modo autenticado, puedes enviar opcionalmente `config[cl_carnet_rut]` para prellenar y bloquear el RUT del usuario durante esa sesión.

```bash title="GET /api/v1/privacy_center/session_token"
curl --request GET \
  --url 'https://api.soyio.com/api/v1/privacy_center/session_token?entity_id=<entity_id>&config[cl_carnet_rut]=<user_rut>' \
