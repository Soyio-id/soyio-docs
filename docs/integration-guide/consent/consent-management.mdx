...
`consentManagement.scopeGroups`: `array` opcional.
...
- `appearance.config.showConsentManagementHeader`: `boolean`.
  ...
  content: {
    consentManagement: {
      header: {
        ...
        description: 'Configura qué autorizaciones deseas mantener activas.'
      }
    }
  }
};
