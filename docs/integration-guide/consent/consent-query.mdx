...
- `complies`: El usuario cumple con todos los requerimientos del template de consentimiento.
...
El payload también expone metadatos para distinguir entre el cumplimiento efectivo y la última acción explícita de consentimiento registrada para ese template. Por ejemplo, si `status` es `complies`, pero `last_consent_action_id` es `null` y `last_related_consent_action_template_version` tiene valor, el Agreement actual cumple con la versión evaluada, aunque la última acción explícita haya quedado registrada en otra versión del mismo template.
