...
:::info[Profundiza]
Revisa la [estructura del Compliance Status](/docs/api/resources/schemas/compliancestatus) en nuestra referencia de API.
:::
...
Este endpoint es útil para consultar los consentmientos de un usuario de forma general y simplificada. Ya que cada template de consentimiento agrupa varias categorías de datos y de usos configurados proviamente para tu empresa.
...
:::info[Profundiza]
Revisa la [estructura del Agreement](/docs/integration-guide/concepts/agreement) para más información o explora el [schema del Agreement](/docs/api/resources/schemas/agreement) en nuestra referencia de API.
:::
...
Obtén la versión más reciente del Agreement de un usuario específico usando el endpoint de listado con filtros y ordenando por `created_at` de forma descendente.
...
// Ejemplo de uso
const agreement = await getAgreement('agr_IdBeyWCa1ENLUYnPRLpCUg');
