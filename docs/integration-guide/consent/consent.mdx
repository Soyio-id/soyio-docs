...
Integra un componente de consentimiento directamente en tus flujos para capturar uno o más consentimientos explícitos de tus usuarios.
...
Cada vez que el usuario realice una acción consentimiento, se **creará un registro de consentimiento mediante un token de acción** (`action_token`). Este token se puede usar para crear un registro de consentimiento en el backend y generar [evidencia del consentimiento otorgado](../main/agreement.mdx).

A grandes rasgos, el módulo de consentimiento te permite:
...
- **Obtener consentimiento múltiple o granular**: Permite obtener el consentimiento de una o más [categorías de uso de datos](../main/taxonomy.mdx#categorías-de-datos-personales).
- **Evidencia vinculante**: Generamos trazabilidad del consentimiento otorgado mediante un [agreement y evidencia](../main/agreement.mdx).
- **Integración fluida**: Ofrecemos dos maneras de integrar el módulo: a través de un componente embebido, o generando los tokens a través de nuestra API.
- **Generar confianza**: En caso de usar nuestro módulo embebido: informamos al usuario sobre el uso de sus datos de manera transparente y clara, utilizando simbología sencilla y dando la opción de más información directamente en el componente.

:::tip
Puedes averiguar más acerca de nuestra simbología y taxonomía en la guía de [Taxonomía y gestión de datos](../main/taxonomy.mdx).
:::

## El proceso de consentimiento

El proceso de consentimiento se compone de los siguientes pasos:
...
6. Para verificar si puedes utilizar un dato para una finalidad específica, siempre debes consultar el [Agreement](../../api/resources/schemas/agreement) actualizado o el [ComplianceStatus](../../api/resources/schemas/compliancestatus) asociado.

:::info[Información]
Los Agreements pueden cambiar a lo largo del tiempo debido a nuevas acciones de consentimiento, modificaciones o revocaciones. Por eso, siempre es recomendable consultar el Agreement actual o su ComplianceStatus correspondiente a través de la API de Soyio.
:::
...
El flujo que integrarás para el enfoque con SDK es el siguiente:
...
Para el enfoque vía API, el flujo es un poco más simple:
  ...
  actions={[
    {
      from: 'frontend',
      to: 'backend',
      label: 'Otorgar consentimiento'
    },
    {
      from: 'backend',
      to: 'soyio_api',
      label: 'Crear action token con consent_template_id'
    },
    {
      from: 'soyio_api',
      to: 'backend',
      label: 'Retorna action_token generado'
    },
    {
      from: 'backend',
      to: 'frontend',
    ...
    },
    {
      from: 'frontend',
      to: 'backend',
      label: 'Submit de formulario / flujo individual'
    },
    {
      from: 'backend',
      to: 'soyio_api',
      label: 'Crear registro de consentimiento con action_token'
    },
    {
      from: 'soyio_api',
      to: 'backend',
...
**Para integración vía API:**
1. Tener acceso a la API de Soyio desde tu backend.
2. Tener una [plantilla de consentimiento](../../api/resources/schemas/consenttemplate) creada.
3. (Opcional) Crear uno o más [documentos de privacidad](../../api/resources/schemas/privacydocument) para asociar a la plantilla de consentimiento.

## Enfoques de integración

Existen dos enfoques principales para integrar el módulo de consentimiento en tu aplicación:

### Enfoque 1: Integración con SDK (Consentimiento embebido)
Este enfoque utiliza el SDK de Soyio para mostrar un componente de consentimiento directamente en tu interfaz. Los `action_token`s se generan automáticamente cuando el usuario interactúa con el componente.

**Ventajas:**
- Integración rápida
- Gestión automática de eventos y `action_token`s
- Descripción detallada de taxonomía y usos
- Soporte para consentimiento granular y unificado

#### Ejemplo

A continuación, te mostramos un ejemplo de integración del componente embebido de consentimiento implementado en un formulario.
...
Puedes utilizar este ejemplo como base para integrar el componente en tu aplicación.


### Enfoque 2: Integración vía API (Consentimiento universal)
Este enfoque utiliza directamente la API de Soyio para crear `action_token`s, en caso de que necesites o quieras generar tu propia interfaz. Es ideal para flujos donde ya tienes tu propia lógica de consentimiento y/o no soportas componentes embebidos.

**Ventajas:**
- Mayor flexibilidad en la implementación
- Control total sobre la experiencia de usuario
- Integración más ligera sin dependencias del SDK

---

## Paso a paso: Integración con SDK
...
:::warning[Importante]
Es importante que la plantilla de consentimiento describa de manera correcta los usos que se harán de los datos y que tenga al menos una finalidad y un periodo de retención. Revisa nuestra sección de [uso de datos](../main/taxonomy.mdx#propósitos-de-procesamiento) para más detalles.
:::
...
## Paso a paso: Integración vía API (Consentimiento universal)
...
El proceso es idéntico al del enfoque con SDK. Deberás crear una [plantilla de consentimiento](../../api/resources/schemas/consenttemplate) que defina qué datos y permisos necesitas solicitar.

:::tip
Para el consentimiento universal vía API, es recomendable usar plantillas de consentimiento unificado ya que generalmente se captura el consentimiento para múltiples categorías de datos en un solo momento.
:::

### 2. Implementar tu propia lógica de consentimiento

Con este enfoque, tienes total libertad para implementar tu propia interfaz y lógica de consentimiento. Sin embargo, **te recomendamos encarecidamente obtener la información de la plantilla de consentimiento** que creaste para mostrar al usuario los textos, usos de datos y categorías de datos correspondientes.
...
- **Texto del consentimiento** (`text`): El texto explicativo que debe mostrarse al usuario
- **Usos de datos** (`data_uses`): Las categorías específicas de uso de datos según nuestra [taxonomía](../main/taxonomy.mdx)
- **Categorías de datos** (`data_categories`): Los tipos de datos que se recolectarán
...
- **Gestión centralizada**: Cambios en el consentimiento se reflejan automáticamente sin modificar código
- **Consistencia**: Mantienes la taxonomía estándar de Soyio para usos y categorías de datos
- **Flexibilidad**: Puedes actualizar textos y permisos desde el API sin deploys
- **Cumplimiento**: Aseguras que la información mostrada coincide exactamente con lo registrado

### 3. Crear el token de acción vía API

Cuando el usuario otorga el consentimiento en tu interfaz, debes crear directamente un token de acción usando el [endpoint de creación de tokens de acción universales](../../api/resources/create-action-token).

```json title="POST /api/v1/universal_consent/action_tokens"
{
...
### 4. Crear el registro de consentimiento

Una vez que el usuario haya otorgado su(s) consentimiento(s) y tengas el `action_token`, procede de una manera similar al enfoque con SDK: crea el registro de consentimiento mediante el [endpoint de creación de acción de consentimiento](../../api/resources/create-consent-action.api.mdx), incluyendo información contextual necesaria del método de captura
...
Puedes también crear un [consent commits](../../api/resources/schemas/consentcommit). si es que quieres registrar varios consentimientos en una misma transacción

---

## Gestión de consentimientos (Ambos enfoques)
...
Desde la [configuración](../../api/resources/schemas/configuration) de tu compañía en Soyio, puedes personalizar el comportamiento del módulo de consentimiento a través de las siguientes opciones:

#### Dominios permitidos (`whitelisted_domains`)

Puedes especificar una lista de dominios permitidos para el contenido de tu embed. Los dominios especificados se agregarán a la lista de dominios permitidos para el contenido de tu embed.

#### Visualización de etiquetas (`label_display`)

Los consentimientos pueden ser opcionales o obligatorios. Puedes configurar cuándo mostrar las etiquetas:
...
#### Texto de etiqueta obligatoria (`mandatory_label_text`)

Cuando `label_display` está configurado como `mandatory_only`, puedes personalizar el texto de la etiqueta:

- **`required`** (por defecto): Muestra "Requerido"
- **`mandatory`**: Muestra "Obligatorio"

Estas configuraciones se aplican a nivel de organización y afectan todos los componentes de consentimiento de tu cuenta.
