...
7. **Link al centro de privacidad de la empresa**: Donde el usuario puede encontrar más información sobre cómo puede revocar su consentimiento o ejercer sus derechos. En los casos donde la empresa no tenga un centro de privacidad, podrá configurar un correo electrónico para que el usuario pueda contactar a la empresa.
...
## Configura los estilos de íconos
...
### Configura el comportamiento cuando el usuario ya ha consentido

Usa las opciones `optionalReconsentBehavior` y `mandatoryReconsentBehavior` para configurar el comportamiento dependiendo si el consentimiento es opcional o obligatorio.

Esto ocurre cuando inicializas el componente de captura de consentimiento con un id de entidad (`entityId`) que ya otorgó consentimiento usando la plantilla de consentimiento que estás usando.

#### Comportamiento de consentimientos opcionales

Configura `optionalReconsentBehavior` con una de las siguientes opciones para indicar el comportamiento cuando el consentimiento es opcional:
...
Configura `mandatoryReconsentBehavior` con una de las siguientes opciones para indicar el comportamiento cuando el consentimiento es obligatorio:
...
Incluye un link al portal de gestión de consentimientos en cada solicitud de consentimiento configurando una url pública para el Privacy Center. Puedes hacerlo usando la opción `privacy_center.public_url` en la [configuración](../../api/resources/schemas/configuration) de tu compañía en Soyio.
