...
- `theme: 'night'` sigue siendo aceptado por compatibilidad, pero se considera deprecado.
:::

<img src="/img/consentimiento-estilos.png" alt="Consent Embed example" width="100%"/>
...
:::tip
Si necesitas sombras (`box-shadow`), configúralas mediante `appearance.rules`. No existe una variable global `shadow`.
:::
...
Los SDKs web y mobile ahora apuntan por defecto al dominio dedicado de consentimiento. Si usas entornos de staging o entornos locales, sigue usando `developmentUrl` para redirigir el embed a tu origen de prueba.
...
<Tabs>
  <TabItem value="web-granular-scope" label="SDK Web" default>
  ```javascript title="Configuración de selección granular"
  const consentOptions = {
    templateId: '<consent_template_id>',
  ...
  <TabItem value="mobile-granular-scope" label="SDK Mobile">
  ```tsx title="Configuración de selección granular"
  const options = {
...
Incluye un link al portal de gestión de consentimientos en cada solicitud de consentimiento configurando una url pública para el Privacy Center. Puedes hacerlo usando la opción `privacy_center.public_url` en la [configuración de tu compañía](../../integration-guide/company-configuration) en Soyio.
...
Cuando la integración del consentimiento es inválida, el componente muestra mensajes más accionables. Los casos más comunes son:

- `consentTemplateId` inválido o inexistente
- host del embed no permitido
- host del embed inválido o mal propagado por la integración

Revisa estas guías si encuentras alguno de esos errores:

- [Gestiona consentimientos](./consent-management)
- [Actualiza la configuración de consentimiento](../../api/resources/update-consent-config.api.mdx)
- [Actualiza la configuración de compañía](../../api/resources/update-company-config.api.mdx)
