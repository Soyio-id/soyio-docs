...
```json
{
  ...
  "identity_id": "id_1B2M2Y8AsgTpgAmY7PhCfg",
...
El módulo de Consentimientos trabaja con **Entities** porque los consentimientos no requieren validación de identidad. Las entidades permiten asociar múltiples consentimientos a un mismo sujeto digital sin identificarlo.

### Gestión de derechos → Entity

El módulo de Solicitudes de Derechos trabaja con **Entities**. El usuario puede ejercer sus derechos de forma anónima o identificado según el `user_reference` de la entidad. Si el proceso de DSR incluye validación de identidad, la entidad obtendrá attestations.
