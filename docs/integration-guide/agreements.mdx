...
*   **Permisos (Data Permissions)**: La lista explícita de autorizaciones vigentes.
...
Los permisos definen el alcance del acceso a los datos. Cada permiso incluye:
...
Un permiso no siempre es global. Puede estar restringido a un contexto específico definido por tres campos:

*   **Scope Type**: El tipo de alcance (`product` o `branch`).
*   **Scope ID**: El identificador (token) del producto o sucursal.
*   **Scope Version**: La versión específica del alcance.

Esto permite que un usuario otorgue consentimiento para un **Producto A** sin que ello implique autorización para todo el ecosistema de la empresa (o viceversa).
...
3.  **Diffing (Cálculo de cambios)**: El sistema calcula qué permisos se agregaron, eliminaron o extendieron respecto a la versión anterior.
4.  **Evidence**: Se genera y asocia una [Evidencia](./evidence-traceability#tipos-de-evidencia) inmutable para auditar el cambio.
...
Son los más comunes en flujos de consentimiento web (cookies, formularios simples).
*   **Caso de uso**: Un usuario entra a tu sitio y acepta cookies.
