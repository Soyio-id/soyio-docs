...
### 5. Consulta y gestión de consentimientos

Una vez creados los consentimientos, puedes realizar las siguientes acciones para gestionar y dar seguimiento a los consentimientos otorgados:

1. **Consultar consentimientos existentes**:
   - Buscar consentimientos específicos filtrando por `entity_id` o `consent_template_id` al [listar los `ConsentActions`](../../api/resources/index-consent-actions).
   - Verificar el estado y vigencia de los consentimientos por usuario.
   - Consultar el historial de consentimientos para auditoría.

2. **Gestionar consentimientos activos**:
   - Evitar crear registros de consentimiento innecesarios o duplicados
   - Actualizar consentimientos cuando sean necesarios
   - Renovar consentimientos próximos a expirar
