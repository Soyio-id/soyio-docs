...
Según el cómo se estructuran los `data_uses` dentro de los [`data_requirements`](../../api/resources/schemas/datarequirement) del `consent_template` creado, se pueden crear dos tipos de consentimientos:

```mermaid
flowchart TD
   A[Data Uses] --> B{¿Mismo nodo raíz?}
   B -->|Sí| C[Consentimiento Granular]
   B -->|No| D[Consentimiento Múltiple]
```

### Consentimiento Granular
Se usa cuando todos los `data_uses` comparten el mismo nodo raíz.
...
### Consentimiento Múltiple
Se usa cuando los `data_uses` provienen de diferentes nodos raíz.
