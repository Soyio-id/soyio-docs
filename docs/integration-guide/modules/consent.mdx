...
### 5. Gestión post-consentimiento

Una vez obtenido el consentimiento del usuario, es importante gestionarlo adecuadamente para mantener la integridad y utilidad de los datos recopilados. A continuación, se presentan las mejores prácticas y herramientas para la gestión post-consentimiento.

1. Para buscar consentimientos creados con ciertos usuario, o plantilla de consentimiento, puedes filtrar por `entity_id` o `consent_template_id` al [listar los `ConsentActions`](../../api/resources/index-consent-actions).
2. Evita crear registros de consentimiento innecesarios o duplicados.
3. Considera usar el [registro agrupado](consent#41-registro-agrupado) cuando tengas más de un módulo de consentimiento en tu interfaz.
2. Agrega un `product_id` al [crear la plantilla de consentimiento](#1-crear-una-plantilla-de-consentimiento) para poder asociar el consentimiento de los datos a un producto en particular.
3. Agrega un `context` al montar el `ConsentBox` para que el [registro de consentiento creado](#4-crea-el-registro-de-consentimiento) quede asociado a un contexto específico.

:::warning[Importante]
Recuerda que cada registro de consentimiento debe tener un propósito claro y justificado. La creación innecesaria de registros puede complicar la gestión y auditoría de los consentimientos.
:::
