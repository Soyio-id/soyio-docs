...
Al momento de crearse una solicitud de ejercicio de derechos, se creará un nuevo [Data subject Request o DSR](../../api/resources/index-data-subject-requests). Este recurso es el que mantendrá la trazabilidad de la solicitud y su estado a medida que se procese la solicitud.
...
- `data_subject_request.created`: Se crea un nuevo [data subject request](../../api/resources/schemas/data-subject-request).
- `data_subject_request.updated`: Se actualiza el estado de un [data subject request](../../api/resources/schemas/data-subject-request).
