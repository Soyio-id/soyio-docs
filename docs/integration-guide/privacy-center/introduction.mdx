...
Facilita la gestión y administración fácil de consentimientos otorgados por tus usuarios a través del [módulo de consentimiento](../consent/consent.mdx) en un solo lugar.

:::info[Información]
Para que este módulo funcione correctamente, es necesario que tengas habilitado el [módulo de consentimiento](../consent/consent.mdx).
...
Una vez que el usuario haga click para otorgar o revocar un consentimiento, se modificará su [agreement](../main/agreement.mdx), ya que cambiarán los `data_uses` asociados en base a los `data_requirements` del consentimiento asociado.
...
:::info[Información]
Para obtener más información de cómo los consentimientos afectan a los `data_uses`, revisa nuestra guía del [módulo de consentimiento](../consent/consent.mdx).
:::
...
Desde la [configuración](../../api/resources/schemas/configuration) de tu compañía, puedes configurar el comportamiento del proceso de validación de identidad:

#### Obligatoriedad de validación de identidad (`data_subject_request.mandatory_validation`)
