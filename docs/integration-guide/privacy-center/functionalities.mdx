...
Opcionalmente, al crear ese token envía `config[cl_carnet_rut]`. Cuando ese valor está presente, el flujo de DSR en modo autenticado prellena y bloquea el campo de RUT, y la API valida que `contact_information.nin` coincida con el RUT configurado.
