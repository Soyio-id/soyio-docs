...
`consentControl`: `'switch'` (default) | `'checkbox'`

### Modo de operación (Batch Mode)

Permite agrupar múltiples cambios de consentimiento y guardarlos en una sola acción.

- `consentMode`: `'immediate'` (default) | `'batch'`
- `showConfirmation`: `boolean`. Muestra un diálogo de confirmación antes de guardar en modo batch.
