...
- Comprobación de que no se ha superado la fecha de expiración del documento
...
4. **Valores por defecto**: Si ninguna de las anteriores tiene configuración, se usan los valores por defecto (`liveness_check: true`, `id_scan_check: true`, `government_check: false`, `id_match_min_level: 3`)
:::

### Umbral de `id_match` (`id_match_min_level`)

Cuando la validación usa `validation_mode: full` (equivalente a requerir `liveness` + `id_scan` y validar también `id_match`), puedes definir el umbral mínimo de coincidencia facial para considerar exitoso el `id_match`.

- Rango permitido: `1` a `10`.
- Valor más alto: más exigente.
- Si `disclosure.id_match_min_level` es `null`, Disclosure usa `validation.id_match_min_level`.
- Este umbral también se aplica cuando Soyio evalúa si una identidad es reutilizable en el flujo "Welcome back".
