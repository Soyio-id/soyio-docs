...
:::info[¿Quieres integrarlo dentro de tu página?]
También puedes usar el flujo embebido con `DisclosureRequestBox`.

Antes de usarlo, debes configurar los dominios host en `whitelisted_domains`, o el navegador bloqueará el flujo por CSP (`frame-ancestors`).

Revisa la guía completa: [Valida una identidad en modo embebido](./embedded-disclosure).
:::
