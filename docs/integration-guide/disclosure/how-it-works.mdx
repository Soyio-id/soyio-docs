...
Este flujo comienza con la entrega de [consentimiento](../consent/introduction). El usuario verá los datos que estás solicitando (definidos en tu [plantilla de disclosure](./disclosure_templates)) y deberá confirmar su consentimiento para continuar.
...
:::danger[Crítico para prevención de fraude]
El match de datos es una **capa esencial de seguridad** para prevenir fraudes en plataformas de comercio. Sin esta validación, un usuario podría:
...
**Si operas un comercio**, evalúa cuidadosamente tu nivel de exposición al fraude antes de decidir no usar matchers. Esta funcionalidad está disponible solo cuando creas el disclosure request desde el backend.
:::

Si el usuario creó una cuenta en tu sistema previamente y entregó datos de identificación, puedes incluir como parte del proceso que Soyio **verifique el match entre los datos verificados** que el usuario entrega en este proceso y la información almacenada en tu sistema. Si usas esta opción y el match no es exitoso, el **proceso concluye sin crear una identidad**.
...
- **Failed**: La solicitud ha fallado porque los datos no hacen match. Este chequeo solo ocurre cuando se realiza un match de datos. El proceso no se completó exitosamente.
...
Si necesitas realizar esta verificación de forma directa desde tu backend sin pasar por el widget, consulta la guía de [verificación gubernamental vía API](./government-check).
