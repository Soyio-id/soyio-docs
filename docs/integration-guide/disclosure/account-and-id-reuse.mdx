...
- **Attestations previos compatibles**: La identidad debe tener los attestations requeridos por la validación configurada (`liveness`, `id_scan` y opcionalmente `government_check`). Cuando además aplica `id_match` (`validation_mode: full`), su `id_match_level` debe ser igual o mayor al `id_match_min_level` exigido.
