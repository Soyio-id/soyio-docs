...
- **Attestations previos compatibles**: La identidad debe tener los attestations requeridos por la validación configurada (`liveness`, `id_scan`, `id_match` cuando `validation_mode: full`, y opcionalmente `government_check`). Cuando aplica `id_match`, su `id_match_level` debe ser igual o mayor al `id_match_min_level` exigido.
...
Si la identidad reutilizable no cumple los attestations requeridos (o no alcanza el umbral de `id_match` cuando corresponde), el sistema no usa el atajo y deriva al flujo completo de validación.
...
<details>
  ...
  graph TD
      ...
      C -- Sí --> J{Attestations previos<br/>compatibles}
      C -- No --> D{Historial con la empresa}
      D -- Sí --> J
      D -- No --> G
      J -- Sí --> E[Muestra pantalla Welcome back]
      J -- No --> G
...
Mientras la autenticación se realiza, el `disclosure_request` permanece en estado `authenticating`. Cuando finaliza con éxito, recibirás el evento `disclosure_request.granted` (igual que en una validación completa) y `auth_attempt.successful` (diferente al que se enviaría en una validación completa).

:::info[Compatibilidad histórica de `id_match_level`]
Si la identidad tiene attestation `id_match` sin `id_match_level`, Soyio la evalúa con compatibilidad histórica usando nivel `3`.
:::
