...
Usa llaves descriptivas como `name`, `last_name`, `date_of_birth` o `cl_carnet_rut`. Estas claves se reconocen automáticamente por la API y también se usan para los `matchers` en [`POST /api/v1/disclosure_requests`](../../api/resources/create-disclosure-request.api.mdx). Si necesitas una clave propia, defínela con claridad y especifica la categoría manualmente.

:::warning[Match de datos y prevención de fraude]
...
**Recomendación:** Define al menos las llaves `cl_carnet_rut`, `name`, `last_name` y `date_of_birth` en tus plantillas para poder usar matchers completos. Aprende más sobre el [match de datos](./how-it-works#match-de-datos) para entender cómo funciona esta validación.
:::
...
:::tip
Si necesitas el detalle completo de parámetros y respuestas, consulta [Crear un template de divulgación](../../api/resources/create-disclosure-template.api.mdx).
:::
...
Usa el endpoint [`POST /api/v1/disclosure_templates`](../../api/resources/create-disclosure-template.api.mdx) para crear tu plantilla con la configuración que definiste.

### Usar la plantilla

Cuando la plantilla esté lista, referencia su `id` para iniciar un proceso de verificación de identidad con [`POST /api/v1/disclosure_requests`](../../api/resources/create-disclosure-request.api.mdx). Si defines `matchers`, asegúrate de que coincidan con las llaves configuradas en los `data_requirements` para evitar errores de validación.

### Gestiona la plantilla

Recuerda actualizar la plantilla con [`PATCH /api/v1/disclosure_templates/{id}`](../../api/resources/update-disclosure-template.api.mdx) cuando cambien los datos solicitados o las finalidades.

:::warning
Solo puedes eliminar una plantilla si no tiene disclosures asociados. Revisa [Eliminar un template de divulgación](../../api/resources/delete-disclosure-template.api.mdx) para más información.
:::
