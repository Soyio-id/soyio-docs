...
Soyio redefine la relación entre usuarios y empresas al introducir los conceptos de **Agreement** y **Evidencia**. Estos conceptos te permiten mantener un registro preciso y cumplir con normativas de protección de datos de forma ágil y confiable, mientras que tus usuarios obtienen un control más transparente y seguro sobre los datos que deciden compartir.

## Agreement

Cuando un usuario interactúa con tu empresa a través de los módulos de Soyio, se genera o actualiza automáticamente un acuerdo digital, denominado **Agreement**. Este acuerdo va más allá de un contrato tradicional: es una representación dinámica de la relación actual entre tu empresa y el usuario. En él se especifica:
- Las categorías de datos que procesarás
- El uso que les darás a los datos
- El alcance del uso de los datos (puede abarcar a toda la empresa o solo un [producto](/docs/api/resources/schemas/product) o solo una [filial](/docs/api/resources/schemas/branch))
- El tiempo de conservación de los datos

El Agreement actúa como un eje central en las soluciones de gestión de consentimiento que ofrece Soyio. Se crea en la primera interacción del usuario contigo y **cada vez que el usuario interactúa nuevamente con módulos relacionados al consentimiento, se genera una nueva versión del agreement, preservando el historial completo de versiones anteriores**. Este sistema de versionado te permite mantener un registro inmutable de la evolución de los consentimientos a lo largo del tiempo.

Este acuerdo unificado proporciona al usuario una experiencia fluida, generando confianza y transparencia, mientras que a ti te proporciona un registro centralizado y fácilmente accesible de todos los consentimientos que ha otorgado cada usuario y cómo se han modificado a lo largo del tiempo, manteniendo la trazabilidad completa del historial de consentimientos.

:::info
Revisa nuestra sección de [taxonomía](./taxonomy) para más información sobre las categorías de datos y las finalidades para las que puedes utilizarlos.
:::
