...
En DSR también puedes ajustar el nivel mínimo requerido para considerar válido el `id_match` cuando usas `validation_mode: full` (equivalente a `liveness_check: true` e `id_scan_check: true`).
