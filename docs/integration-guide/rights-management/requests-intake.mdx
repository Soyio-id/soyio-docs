...
Debes enviar `entity_id` como query param. Opcionalmente, puedes enviar `config[cl_carnet_rut]` para fijar el RUT del usuario durante la sesión autenticada.

<Tabs>
  <TabItem value="web" label="SDK Web" default>
```javascript title="Backend - Obtener token de sesión"
const getSessionToken = async (entityId, userRut) => {
  const url = new URL('https://api.soyio.com/api/v1/privacy_center/session_token');
  url.searchParams.append('entity_id', entityId);

  if (userRut) {
    url.searchParams.append('config[cl_carnet_rut]', userRut);
  }
...
:::caution Importante
Si generas el token con `config[cl_carnet_rut]`, el campo RUT del formulario DSR quedará prellenado y bloqueado en el Privacy Center. Además, al crear la solicitud, `contact_information.nin` debe coincidir con ese valor; en caso contrario, la API responderá `400`.
:::

:::tip
Revisa la referencia del API para [Obtener un token de sesión para el centro de privacidad](../../api/resources/get-privacy-center-session-token.api.mdx) para mas detalles.
:::
