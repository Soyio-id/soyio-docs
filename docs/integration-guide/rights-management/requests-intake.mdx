...
Debes enviar `entity_id` como query param. Opcionalmente, envía `config[cl_carnet_rut]` para fijar el RUT del usuario durante la sesión autenticada.
...
:::tip
Revisa la referencia del API para [Obtener un token de sesión para el centro de privacidad](../../api/resources/get-privacy-center-session-token.api.mdx) para más detalles.
:::
