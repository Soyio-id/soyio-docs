...
test.describe.parallel('Basic Navigation', () => {
  ...
  test('Visit the main page', async ({ page }) => {
    await expect(
      page.locator('.navbar__title').getByText('Soyio Docs'),
    ).toBeVisible();
  });

  test('Navigate to Integration Guides', async ({ page }) => {
    await page
      .locator('.navbar__item')
      .getByText('Guías de integración')
      .click();
    await expect(page).toHaveURL(/\/docs\/integration-guide\/intro/);
    await expect(
      page.getByRole('heading', { name: 'Guías de integración' }),
    ).toBeVisible();
  });
  ...
  test('Navigate to API Reference', async ({ page }) => {
    await page
      .locator('.navbar__item')
      .getByText('Referencia de la API')
      .click();
  ...
  test('Navigate to User Guides', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Guías de usuario' }),
    ).toBeVisible();
  });
});
